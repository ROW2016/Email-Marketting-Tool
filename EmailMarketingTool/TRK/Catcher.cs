...
namespace TRK
{
    public class Catcher : IHttpModule
    {
        ...
        private string pattern = @"/image/(?<key>.*)\.png";
        ...
        public void GetImage_BeginRequest(object sender, System.EventArgs args)
        {
            ...
            if (r.IsMatch(url))
            {
                ...
                if ((mc != null) && (mc.Count > 0))
                {
                    ...
                    {
                        client.DownloadFile(logoFile + imgName, application.Request.MapPath("\\Logos\\" + SIdentifier+".png"));
                    }
                }
                //send the REAL image to the client
                application.Response.ContentType = "image/png";
                application.Response.WriteFile(application.Request.MapPath("\\Logos\\" + SIdentifier + ".png"));
